...
void val_para_list(char name[]) {
	 ...
	 do{
		 ...
		 if (sym == IDENSY || sym == CHAR2SY || int_judge() || sym == LPARSY) {
			expr();
			if (factor_type != stable.list[i].type) {
	...
	if (sym == DEFAULTSY) {
		...
		if (sym == COLONSY) {
			...
			if (sym == RBPARSY) {
				...
				statement();
				append_mid_code(25, mid_empty, mid_empty, switch_label);
			}
		}
		else {
	...
	if (sym == LPARSY) {
		...
		if (sym == RPARSY) {
			...
			if (sym == LBPARSY) {
				...
				if (sym == RBPARSY) {
					sym = getsym();
					append_mid_code(33, mid_empty, mid_empty, switch_label);
