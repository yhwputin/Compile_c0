...
void append_mid_code(int op, char src1[], char src2[], char result[]) {
	...
	m_table.mid_list[m_table.mid_num].use = 1;
	m_table.mid_num++;
}
void print_mid_code(FILE *mid_code_file) {
	int i;
	for (i = 0; i <m_table.mid_num; i++) {
		if (m_table.mid_list[i].use == 0) {
			continue;
		}
		print_one_mid_code(i,mid_code_file);
	}
	fclose(mid_code_file);
}
void print_one_mid_code(int number,FILE * mid_code_file) {
	int i;
